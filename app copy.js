...
(() => {
  ...
  // Interactions
  let mouse = { x: 0, y: 0, wx: 0, wy: 0, down: false, button: 0, drag: false, start: { x: 0, y: 0, wx: 0, wy: 0 } };
  let panning = false;
  let shiftHeld = true;
  ...
  function pushState() {
    const snap = polygons.map(p => ({ room: p.room.roomName, coords: p.coords.map(q => [q[0], q[1]]) }));
    undoStack.push(snap);
  ...
  document.getElementById('loadFile').addEventListener('change', async (e) => {
    ...
    try {
      ...
      const rooms = (data.wardInfo && data.wardInfo.coreRoomLayout) || [];
      polygons = rooms.map(room => {
        const coords = room.vertices.map(v => [Number(v.X), Number(v.Y)]);
        const lineColor = randomColor();
        return { room, coords, color: lineColor, selected_vertex: null, selected_edge: null };
      });
      fitView(); draw(); toast('Loaded ' + polygons.length + ' rooms');
    } catch (err) { console.error(err); toast('❌ Invalid JSON'); }
  });
  ...
  // Controls
  document.getElementById('gridToggle').addEventListener('change', e => { grid.show = e.target.checked; draw(); });
  document.getElementById('alignToggle').addEventListener('change', e => { alignEnabled = e.target.checked; });
  document.getElementById('snapStep').addEventListener('input', e => { const v = Number(e.target.value) || 1; snapStep = v; draw(); });

  // Keyboard
  window.addEventListener('keydown', (e) => {
    if (e.key === 'Shift') shiftHeld = true;
    else if (e.key === 'g' || e.key === 'G') { snapEnabled = !snapEnabled; toast(`Grid snap: ${snapEnabled ? 'ON' : 'OFF'}`); }
    else if (e.key === 'r' || e.key === 'R') { if (selected) { pushState(); const nn = prompt(`Enter new name for '${selected.room.roomName}':`, selected.room.roomName); if (nn) { selected.room.roomName = nn; draw(); } } }
    else if (e.key === 's' || e.key === 'S') { saveJSON(); }
    else if (e.key === 'u' || e.key === 'U' || (e.ctrlKey && e.key.toLowerCase() === 'z')) { undo(); }
  });
  ...
  canvas.addEventListener('mousedown', (e) => {
    ...
    mouse.down = true; mouse.button = e.button; mouse.drag = false; mouse.start = { x: mouse.x, y: mouse.y, wx: w.x, wy: w.y };

    if (e.button === 1 || e.button === 2) { panning = true; return; }

    const HIT_VERTEX_TOL = 0.4;
    selected?.coords && (selected.selected_vertex = nearestVertex(selected, [w.x, w.y], HIT_VERTEX_TOL));
    if (selected && selected.selected_vertex != null) { pushState(); return; }

    const edgeIdx = selected ? nearestEdge(selected, [w.x, w.y]) : null;
    if (selected && edgeIdx != null) { selected.selected_edge = edgeIdx; pushState(); return; }

    const poly = polygons.find(p => pointInPoly({ x: w.x, y: w.y }, p));
    if (poly) { selected = poly; draw(); return; }
    selected = null; draw();
  });
  ...
  canvas.addEventListener('mousemove', (e) => {
    ...
    if (selected) { highlightEdge.idx = nearestEdge(selected, [w.x, w.y]); highlightEdge.poly = selected; }
    else { highlightEdge.idx = null; highlightEdge.poly = null; }

    if (!mouse.down) { draw(); return; }

    mouse.drag = (Math.abs(mouse.x - mouse.start.x) > 2 || Math.abs(mouse.y - mouse.start.y) > 2);

    if (panning) {
      const dx = (mouse.x - mouse.start.x) / view.scale; const dy = (mouse.y - mouse.start.y) / view.scale;
      view.x = mouse.start.wx - dx; view.y = mouse.start.wy - dy; draw(); return;
    }

    if (!selected) return;

    const snapped = (v) => snapEnabled ? Math.round(v / snapStep) * snapStep : v;
    function align(x, y, current) {
      if (!alignEnabled) return [x, y];
      for (const poly of polygons) { if (poly === current) continue; for (const [vx, vy] of poly.coords) { if (Math.abs(vx - x) < SNAP_TOL) x = vx; if (Math.abs(vy - y) < SNAP_TOL) y = vy; } }
      return [x, y];
    }

    if (selected.selected_vertex != null) {
      let x = snapped(mouse.wx), y = snapped(mouse.wy);
    ...
    if (selected.selected_edge != null) {
      const idx = selected.selected_edge; const coords = selected.coords;
      const p1 = coords[idx]; const p2 = coords[(idx + 1) % coords.length];
      ...
      let np1 = [p1[0] + shift[0], p1[1] + shift[1]];
      let np2 = [p2[0] + shift[0], p2[1] + shift[1]];
      [np1[0], np1[1]] = align(np1[0], np1[1], selected);
      [np2[0], np2[1]] = align(np2[0], np2[1], selected);
      coords[idx] = np1; coords[(idx + 1) % coords.length] = np2;
      draw(); return;
    }
  });

  window.addEventListener('mouseup', () => {
    if (selected) { selected.selected_vertex = null; selected.selected_edge = null; }
    mouse.down = false; panning = false; draw();
  });

  canvas.addEventListener('wheel', (e) => {
    const { clientX, clientY } = e; const rect = canvas.getBoundingClientRect();
    const x = clientX - rect.left, y = clientY - rect.top;
    const worldBefore = screenToWorld(x, y);
    const delta = -e.deltaY;
    const k = Math.exp(delta * 0.0012);
    view.scale = Math.min(100, Math.max(0.02, view.scale * k));
  ...
  function nearestEdge(poly, p) {
    const cs = poly.coords; if (!cs.length) return null;
    const lens = cs.map((_, i) => {
      const a = cs[i], b = cs[(i + 1) % cs.length]; return Math.hypot(b[0] - a[0], b[1] - a[1]);
    });
  ...
  function drawGrid() {
    if (!grid.show) return;
    const xlim = screenToWorld(0, 0).x; const ylim = screenToWorld(0, canvas.height / DPR).y;
    const xhi = screenToWorld(canvas.width / DPR, 0).x; const yhi = screenToWorld(0, 0).y;
    const step = Math.max(0.2, snapStep);
    const startX = Math.floor(xlim / step) * step; const endX = Math.ceil(xhi / step) * step;
    const startY = Math.floor(ylim / step) * step; const endY = Math.ceil(yhi / step) * step;

    ctx.lineWidth = 1; ctx.strokeStyle = 'rgba(148,163,184,0.15)';
    ctx.beginPath();
    for (let x = startX; x <= endX; x += step) { const s = worldToScreen(x, 0); ctx.moveTo(s.x, 0); ctx.lineTo(s.x, canvas.height / DPR); }
    for (let y = startY; y <= endY; y += step) { const s = worldToScreen(0, y); ctx.moveTo(0, s.y); ctx.lineTo(canvas.width / DPR, s.y); }
    ctx.stroke();
  }
  ...
  function draw() {
    ...
    for (const poly of polygons) {
      ...
      // fill
      ctx.beginPath();
      poly.coords.forEach(([x, y], i) => { const s = worldToScreen(x, y); if (i === 0) ctx.moveTo(s.x, s.y); else ctx.lineTo(s.x, s.y); });
      ctx.closePath();
      ...
      // vertices
      for (const [x, y] of poly.coords) { const s = worldToScreen(x, y); ctx.beginPath(); ctx.arc(s.x, s.y, 3, 0, TAU); ctx.fillStyle = '#cbd5e1'; ctx.fill(); }

      // centroid label
      const c = centroid(poly); const sc = worldToScreen(c.x, c.y);
    ...
    if (highlightEdge.poly && highlightEdge.idx != null) {
      const cs = highlightEdge.poly.coords; const i = highlightEdge.idx;
      const a = worldToScreen(cs[i][0], cs[i][1]);
      const b = worldToScreen(cs[(i + 1) % cs.length][0], cs[(i + 1) % cs.length][1]);
      ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y);
      ctx.strokeStyle = 'rgba(250,204,21,0.9)'; ctx.lineWidth = 3; ctx.stroke();
    }
  }

  function fitView() {
    if (!polygons.length) { view.x = 0; view.y = 0; view.scale = 20; return; }
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    for (const p of polygons) { for (const [x, y] of p.coords) { minX = Math.min(minX, x); minY = Math.min(minY, y); maxX = Math.max(maxX, x); maxY = Math.max(maxY, y); } }
    const pad = 1;
  ...
  function randomColor() {
    const h = Math.random(); const s = 0.6, l = 0.55;
...
(() => {
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    if (selected.selected_edge != null) {
      ...
      [np2[0], np2[1]] = align(np2[0], np2[1], selected);
      coords[idx] = np1;
      coords[(idx + 1) % coords.length] = np2;
      draw();
      return;
    }
  });
  ...
  function drawGrid() {
    ...
    const ylim = screenToWorld(0, canvas.height / DPR).y;
    const xhi = screenToWorld(canvas.width / DPR, 0).x;
    const yhi = screenToWorld(0, 0).y;
    const step = Math.max(0.2, snapStep);
...
(() => {
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    function align(x, y, current) {
      if (!alignEnabled || !current) return [x, y];

      const zoomFactor = 1 / Math.max(0.01, view.scale);
      const avgRoomSize = (() => {
        let total = 0,
          count = 0;
        for (const p of polygons) {
      ...
      // === Adaptive tolerances ===
      const SNAP_TOL = 0.25 * (zoomFactor + avgRoomSize * 0.002);
      const ANGLE_TOL = 0.996; // cosine threshold (~5°)
      const MAX_EDGE_CHECK = Math.max(
        500,
        Math.min(6000, polygons.length * 30)
      );

      // Must be dragging an edge for room-to-room alignment
      if (current.selected_edge == null) return [x, y];

      const idx = current.selected_edge;
      ...
      if (edgeLen < 1e-8) return [x, y];

      const edgeDir = [edgeVec[0] / edgeLen, edgeVec[1] / edgeLen];
      const edgeNormal = [edgeDir[1], -edgeDir[0]];
      ...
      for (const poly of polygons) {
        ...
        for (let i = 0; i < cs.length; i++) {
          if (++checked > MAX_EDGE_CHECK) break;
          ...
          const abDir = [abx / abLen, aby / abLen];
          const abNormal = [abDir[1], -abDir[0]];

          // Are the walls roughly parallel?
          const dot = Math.abs(edgeDir[0] * abDir[0] + edgeDir[1] * abDir[1]);
          if (dot < ANGLE_TOL) continue;

          // Facing direction check — normals should point toward each other
          const facing =
            edgeNormal[0] * abNormal[0] + edgeNormal[1] * abNormal[1] < 0;
          if (!facing) continue;

          // Compute perpendicular distance between infinite lines
          const offset =
            (a[0] - p1[0]) * edgeNormal[0] + (a[1] - p1[1]) * edgeNormal[1];
          const absOffset = Math.abs(offset);

          // Check proximity within adaptive tolerance
          if (absOffset < SNAP_TOL && absOffset < minDist) {
            // Now check overlapping projection along the edge direction
            const edgeCenter = [(p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2];
            const otherCenter = [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2];
            const projDist = Math.abs(
              (edgeCenter[0] - otherCenter[0]) * edgeDir[0] +
                (edgeCenter[1] - otherCenter[1]) * edgeDir[1]
            );
            if (projDist < edgeLen * 0.7) {
              // edges roughly overlap
              bestAlign = { offset, nx: edgeNormal[0], ny: edgeNormal[1] };
              minDist = absOffset;
            }
          }
        }
      ...
      // === Apply perfect flush alignment ===
      if (bestAlign) {
        const { offset, nx, ny } = bestAlign;
        const shiftX = -nx * offset;
        const shiftY = -ny * offset;

        // Move both vertices of the selected wall
        coords[idx][0] += shiftX;
        coords[idx][1] += shiftY;
        coords[(idx + 1) % coords.length][0] += shiftX;
        coords[(idx + 1) % coords.length][1] += shiftY;

        draw();
        return [x + shiftX, y + shiftY];
      }

      // === fallback (point alignment) ===
      ...
      for (const poly of polygons) {
        ...
        for (let i = 0; i < cs.length; i++) {
          ...
          if (dist < SNAP_TOL && dist < best.dist) {
            best = { x: cx, y: cy, dist };
          }
        }
      }
...
//------- WIth room names, rooms align, but the edge shifting not implemented yet
...
(() => {
  ...
  canvas.addEventListener(
    ...
    (e) => {
      ...
      const delta = -e.deltaY;
      const k = Math.exp(delta * 0.0012);
      view.scale = Math.min(500, Math.max(0.02, view.scale * k));
      const worldAfter = screenToWorld(x, y);
      view.x += worldBefore.x - worldAfter.x;
      view.y += worldBefore.y - worldAfter.y;
      e.preventDefault();
...
//----------------better edge shifting, but not perferct
...
(() => {
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === "Shift") shiftHeld = true;
      ...
      if (selected && highlightEdge.poly === selected && highlightEdge.idx != null) {
        const idx = highlightEdge.idx;
        const coords = selected.coords;

        // Compute midpoint of the edge
        const [x1, y1] = coords[idx];
        const [x2, y2] = coords[(idx + 1) % coords.length];
        const mid = [(x1 + x2) / 2, (y1 + y2) / 2];

        // Insert new vertex into coordinates
        coords.splice(idx + 1, 0, mid);

        toast("🟢 Vertex added on edge");
        draw();
      } else {
        toast("⚠️ Select a room and hover over an edge first");
      }
    }
...
//columns added and made them immovable 
// app.js - modulelet
(() => {
  /*** Utility ***/
  const TAU = Math.PI * 2;
...
(() => {
  ...
  function moveSharedEdge(movingPoly, edgeIdx, shift) {
    ...
    // === STEP 1: Calculate proposed new positions ===
    let newA = [p1[0] + shift[0], p1[1] + shift[1]];
    let newB = [p2[0] + shift[0], p2[1] + shift[1]];

    // === STEP 2: Alignment logic (parallel edge snapping) ===
    if (alignEnabled) {
      const dir = [newB[0] - newA[0], newB[1] - newA[1]];
      const len = Math.hypot(dir[0], dir[1]);
      if (len > 1e-6) {
        ...
        const SNAP_DIST = 0.1;
        const ANGLE_TOL = 0.05; // ≈ 3 degrees
    ...
    // === STEP 3: Check min width / height restriction ===
    ...
    if (constraint) {
      ...
      if (
        ...
        toast(
          `⚠️ '${movingPoly.room.roomName}' cannot be smaller than ${constraint.min_height}m in height`
        );
      ...
      if (
        ...
        toast(
          `⚠️ '${movingPoly.room.roomName}' cannot be smaller than ${constraint.min_width}m in width`
        );
        return; // stop movement
      }
    }

    // === STEP 4: Apply shift to selected and shared polygons ===
    if (!moveSharedEdgesEnabled || !sharedPolys || sharedPolys.length === 1) {
...
// cannot extend beyound bounding box of wardInfo
// app.js - modulelet
(() => {
  /*** Utility ***/
  const TAU = Math.PI * 2;
  ...
  function moveSharedEdge(movingPoly, edgeIdx, shift) {
    ...
    // 🔸 (a) Restrict shrinking smaller than minimum size
    if (constraint) {
      if (
        isHorizontal &&
        constraint.min_height > 0 &&
...
(() => {
  ...
  function moveSharedEdge(movingPoly, edgeIdx, shift) {
    ...
    // === FIXED: Snap to minimum allowed instead of blocking ===
    if (constraint) {
      // Compute current dimensions BEFORE move
      const currXs = movingCoords.map(([x]) => x);
      const currYs = movingCoords.map(([_, y]) => y);
      const currW = Math.max(...currXs) - Math.min(...currXs);
      const currH = Math.max(...currYs) - Math.min(...currYs);
      ...
      // Update testCoords with the corrected positions for subsequent checks
      testCoords[edgeIdx] = newA;
      testCoords[(edgeIdx + 1) % testCoords.length] = newB;
    }
...
// applied edge constraints to shared edge moving also 
// app.js - modulelet
(() => {
  /*** Utility ***/
  const TAU = Math.PI * 2;
...
// added logic to align with other rooms while moving shared edge 
// app.js - modulelet
(() => {
  /*** Utility ***/
  const TAU = Math.PI * 2;
...
// crosshair horizantal and vertical lines added
// app.js - modulelet
(() => {
  /*** Utility ***/
  const TAU = Math.PI * 2;
